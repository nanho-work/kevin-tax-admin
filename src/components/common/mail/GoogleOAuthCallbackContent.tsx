...
type Props = {
  accountPath: string
  completeGoogleOAuth: (code: string, state: string) => Promise<OAuthCompleteResponse>
  listMailAccounts: (includeInactive?: boolean) => Promise<unknown>
...
export default function GoogleOAuthCallbackContent({
  ...
  const getOAuthErrorMessage = (error: unknown): string => {
    ...
    return getMailErrorMessage(error)
  }
  ...
  useEffect(() => {
    ...
    void run()
  }, [params, router, completeGoogleOAuth, listMailAccounts, getMailErrorMessage, accountPath])
