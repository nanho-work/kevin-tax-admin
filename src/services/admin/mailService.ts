...
export async function markMailMessageSpam(
  ...
  const res = await adminHttp.post<{ message: string; mail_message_id: number }>(
...
export async function removeSpamSender(senderEmail: string): Promise<{ message: string }> {
  const res = await adminHttp.post<{ message: string }>(`${BASE}/spam/remove-sender`, {
...
export async function blockSpamDomain(payload: {
  ...
  const res = await adminHttp.post<{ message: string }>(`${BASE}/spam/block-domain`, payload)
  return res.data
}
...
export async function purgeSpamMessage(
  ...
  const res = await adminHttp.delete<{ message: string; mail_message_id: number }>(
