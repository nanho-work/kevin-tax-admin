...
const BASE = `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/notifications/`
...
export function getAdminNotificationErrorMessage(error: unknown): string {
...
export async function listAdminNotifications(params?: {
  ...
  const res = await adminHttp.get(BASE, {
    params: {
      limit,
      offset,
...
export async function markAdminNotificationRead(receiptId: number): Promise<void> {
  await adminHttp.patch(`${BASE}${receiptId}/read`, {})
}

export async function markAllAdminNotificationsRead(): Promise<number> {
  const res = await adminHttp.patch<{ updated_count?: number }>(`${BASE}read-all`, {})
  return Number(res.data?.updated_count ?? 0)
}

export async function fetchAdminNotificationUnreadCount(): Promise<number> {
  const res = await adminHttp.get<PortalNotificationUnreadCountResponse | { count?: number }>(`${BASE}unread-count`)
